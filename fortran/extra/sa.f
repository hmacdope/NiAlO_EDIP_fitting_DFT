...
C
C   This implementation of simulated annealing was used in "Global Optimizatio
...
C  it minimizes by maximizing the negative a function.
     ...
     3          /,''  POOR.'',/)')
      ...
      SUBROUTINE PRT8(N,VM,XOPT,X,f)

      DOUBLE PRECISION  VM(*), XOPT(*), X(*),f
      INTEGER  N
...
C  elements in VECTOR, 10 elements will be printed on each line.
      ...
      IF (NCOLS .GT. width) THEN
         LINES = INT(NCOLS/real(width))

         DO 100, I = 1, LINES
            LL = width*(I - 1)
            WRITE(*,1000) (VECTOR(J),J = 1+LL, width+LL)
  100    CONTINUE

         WRITE(*,1000) (VECTOR(J),J = width+1+LL, NCOLS)
      ELSE
         WRITE(*,1000) (VECTOR(J),J = 1, NCOLS)
      END IF

 1000 FORMAT(30(G12.5,1X))
 1001 FORMAT(/,25X,A)
